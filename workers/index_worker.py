...
def work():
    src_curr.execute('SELECT id, name,requests, CRC32(category) AS category, length, UNIX_TIMESTAMP(create_time) AS create_time, ' +
        'UNIX_TIMESTAMP(last_seen) AS last_seen,requests FROM search_hash WHERE tagged=false LIMIT 10000')
    total = src_curr.rowcount
    print 'fetched', total
    for one in src_curr:
        ret = dst_curr.execute('insert into rt_main(id,name,category,length,create_time,last_seen,requests) values(%s,%s,%s,%s,%s,%s,%s)',
            (one['id'], one['name'], one['category'], one['length'], one['create_time'], one['last_seen'],one['requests']))
        if ret:
